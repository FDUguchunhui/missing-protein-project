...
MP_204_gene_symbol <- missing_proteins$accession
MP_85_gene_symbol <- missing_proteins %>% filter(TPM > 0) %>% select(accession) %>% pull() %>% unique()
```
...
```{r}
primary_cell_nasf_gene_symbol <- primary_cell_nasf_gene_symbol[, unlist(IPAS)]
```


# check cancer-specificity in primary cell
```{r}
all_cancer_primary_cell <- SpC_List(df=primary_cell_nasf_gene_symbol, annotation=primary_cell_annotation, proteins_filter=MP_204_gene_symbol)
```


```{r, fig.height=12, fig.width=8}
plot_heat_map(all_cancer_primary_cell, cluster_cols = F, show_rownames = T, fontsize_row=7,
              main='Cancer-specificity for the identied 85 missing protein')
```
