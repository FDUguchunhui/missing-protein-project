...
# knitr::opts_knit$set(root.dir = '/Users/cgu3/Library/CloudStorage/OneDrive-InsideMDAnderson/data/missing-protein-project/data/')
...
```{r}
# gastric_data_path <- 'gastric-primary-cell/FDR1_distinct2_pep/'
# gastric_data_path <- 'gastric-primary-cell/FDR4-swiss-only/'
gastric_data_path <- 'gastric-primary-cell/FDR4-2017/'
```
...
gastric_nsaf <- as.matrix(read.csv(paste0(gastric_data_path, 'gastric_primary_cell_nsaf_2.csv'), row.names = 1))
# # import gastric primary cell SpC data
gastric_count <- read_csv(paste0(gastric_data_path, 'gastric_primary_cell_count.csv'), show_col_types = FALSE)
...
# dict_accession_to_gene_symbol <- read_csv(paste0(gastric_data_path, 'id_mapping_table.csv'))
```
...
col_name_mapping <- tibble(ipas = c("IP0981_1701","IP0982_1701", "IP0993_1701", "IP0995_1701", "IP0999_1701", "IP7100_1701", "IP7103_1701", "IP7105_1701"),
       sample_id = paste('Sample', 1:8, sep = '_'))
colnames(count_TPM_gene_symbol) <- mapping(colnames(count_TPM_gene_symbol), col_name_mapping)
...
write_csv(full_table_detected_MP, 'detected_38_MP_products_with_RNA.csv')
```
...
(c <- summarize_by_case(combined_long, protein_type = 'MP', group_by_IPAS = FALSE, cases = c(1, 2), remove_IPAS = c('Sample_1')))
```
...
dat_hist$IPAS <- factor(dat_hist$IPAS, levels = c(paste('Sample', 1:8, sep = '_'), "Combined w.o. outlier",  "Combined"))
...
c_RP <- a_RP %>% filter(!IPAS %in% c('Sample_1')) %>% 
  group_by(case) %>% 
    summarise(count= sum(count)) %>%
    mutate(countT=sum(count)) %>% 
    mutate(per=paste0(round(100*count/countT,2),'%'))
...
```{r}
b_RP$IPAS = 'Combined'
c_RP$IPAS = 'Combined w.o. outlier'
...
# dat_hist <- dat_hist[!dat_hist$IPAS %in% c('Combined', 'Combined w.o. outlier'), ]
