...
```{r}
dat_long_NSAF_TPM <- left_join(gastric_nsaf_df_long, count_TPM_gene_symbol_df_long, by=c('IPAS', 'gene_symbol'))
dat_long_NSAF_TPM$NSAF <- ifelse(is.na(dat_long_NSAF_TPM$NSAF), 0, dat_long_NSAF_TPM$NSAF)
dat_long_NSAF_TPM$TPM <- ifelse(is.na(dat_long_NSAF_TPM$TPM), 0, dat_long_NSAF_TPM$TPM)
```
...
Unique accession
```{r}
dat_long_NSAF_TPM %>% filter(NSAF > 0, TPM > 0, type=='MP') %>% select(accession) %>% pull() %>% unique()
```


```{r}
dat_long_NSAF_TPM %>% filter(NSAF > 0, TPM > 0, type=='MP') %>% select(gene_symbol) %>% pull() %>% unique()
```
...
```{r}
combined_long <- dat_long_NSAF_TPM  %>% filter(!(TPM == 0 & NSAF == 0))
```
...
full_table_detected_MP <- combined_long %>% filter(accession %in% missing_proteins) %>% filter(NSAF > 0 & TPM > 0) %>% arrange(accession, IPAS)
...
summarize_by_case(combined_long, protein_type = 'MP', group_by_IPAS = TRUE, cases = c(1, 3))

## overall without removing the outlier
summarize_by_case(combined_long, protein_type = 'MP', group_by_IPAS = FALSE, cases = c(1, 3))

## overall with removing the outlier
summarize_by_case(combined_long, protein_type = 'MP', group_by_IPAS = FALSE,  cases = c(1, 3), remove_IPAS = c('IP0981_1701', 'IP7103_1701'))
```
...
```{r}
glmm_dat_missing <- dat_long_NSAF_TPM %>% filter(type=='MP') %>% mutate(TPM = log(TPM + 1), NSAF = log(NSAF + 1))
glmm_dat_missing_filtered <- glmm_dat_missing %>% filter(TPM > 0 & NSAF > 0)


glmm_dat_non_missing <-dat_long_NSAF_TPM %>% filter(type=='RP') %>% mutate(TPM = log(TPM + 1), NSAF = log(NSAF + 1))
